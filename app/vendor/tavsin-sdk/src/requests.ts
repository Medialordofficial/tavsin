...
export function decimalAmountToBaseUnits(
  ...
  const baseUnits = `${wholePart || "0"}${paddedFraction}`.replace(
    /^0+(?=\d)/,
    ""
  );
...
export function buildSplTransferCheckedPayload(args: {
  ...
  const payload = buildRequestPayloadFromInstruction(
    instruction,
    args.walletPda
  );
...
export async function buildSplTransferCheckedPayloadFromRequest(args: {
  ...
  const amount =
    typeof args.request.amount === "bigint" ||
    typeof args.request.amount === "number"
      ? args.request.amount
